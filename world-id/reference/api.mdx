...
|-----------|------|----------|---------|-------------|
| `nullifier_hash` | `string` | **Yes** | - | The unique user identifier (called the nullifier hash in the ZKP), as provided by IDKit. See [IDKit response](/world-id/reference/idkit#types) for details. |
| `proof` | `string` | **Yes** | - | The zero-knowledge proof, as provided by IDKit. See [IDKit response](/world-id/reference/idkit#types) for details. |
| `merkle_root` | `string` | **Yes** | - | Part of the ZKP, the hash of the Merkle root that proves membership to the set of credentials. As provided by IDKit. See [IDKit response](/world-id/reference/idkit#types) for details. |
| `verification_level` | `string` | **Yes** | - | The verification level, as provided by IDKit. See [IDKit response](/world-id/reference/idkit#types) for details. |
| `action` | `string` | **Yes** | - | Same action identifier as passed to IDKit. |
| `signal_hash` | `string` | No | `hashToField('').digest` | The hash of the signal that was used to generate the proof. Defaults to the hash of an empty string. |
...
```json title="Response - 400 Invalid Proof"
{
	"code": "invalid_proof",
...
```json title="Response - 400 Invalid Merkle Root"
{
...
```json title="Response - 400 Invalid Credential Type"
{
	"code": "invalid",
...
```json title="Response - 400 User Exceeded Maximum Verifications"
{
...
```json title="Response - 400 User Already Verified"
{
...
| Parameter | Type | Required | Default | Description |
|-----------|------|----------|---------|-------------|
| `action` | `string` | **Yes** | `""` | The action to check. This is the same action that was passed to IDKit. Defaults to an empty string for Sign in with World ID. |
| `nullifier_hash` | `string` | No | `""` | The ZKP's nullifier hash, as provided by IDKit. See [IDKit response](/world-id/reference/api#response) for details. Defaults to an empty string. |
| `external_nullifier` | `string` | No | - | **Deprecated.** The ZKP's external nullifier. This parameter is deprecated and will be removed in a future release, and should no longer be used. Instead, pass the `action` parameter. |
...
|-------|------|---------|-------------|
| `__typename` | `string` | `"app"` | Always returns `app`. |
| `id` | `string` | - | The `app_id` from the Developer Portal. |
| `engine` | `"cloud" \| "on-chain"` | - | Whether the app is configured for use with **Cloud** or **On-Chain** proof verifications. |
| `is_staging` | `boolean` | - | Whether the app is a staging app. If `true`, the [Worldcoin Simulator](https://simulator.worldcoin.org) must be used to verify. |
| `is_verified` | `string` | - | Whether the app has been verified by Worldcoin. |
| `name` | `string` | - | The app's name as configured in the Developer Portal. |
| `verified_app_logo` | `string` | - | The URL of the app's logo. Only returned if the app has been verified, otherwise returns an empty string. |
| `is_sign_in` | `boolean` | - | Whether the action request is for Sign in with World ID. |
| `can_user_verify` | `"yes" \| "no" \| "on-chain" \| "undetermined"` | - | If the user is eligible to verify for this action. |
| `action` | `JSON` | - | Information about the action being verified. |

#### `can_user_verify` values:
